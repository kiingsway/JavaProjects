...
public class DBBatchAddController {
    ...
    if (!jsonString.isEmpty()) {
      try {
        ...
        if (insertedCount == 0) {
          ...
          Constants.GO_DATABASE(view, updateTranslateList);
        }
      } catch (Exception er) {
  ...
  private void closeApp() {
    if (!view.txaJson().getText().isEmpty()) {
      ...
      int response = JOptionPane.showConfirmDialog(view, msg, Constants.APP_NAME, JOptionPane.YES_NO_OPTION);
      if (response == JOptionPane.NO_OPTION) return;
    }
    if (updateTranslateList == null) view.dispose();
    else Constants.GO_DATABASE(view, updateTranslateList);
  }
}
